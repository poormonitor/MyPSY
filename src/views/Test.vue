...
const fetchData = () => {
    userOption.length = 0;
    axios
        .get("./tests/" + route.params.id + ".json")
        .then(function (response) {
            testContent.data = response.data;
            if (
                answerSheet.answer.length &&
                route.params.id == answerSheet.testContent.id
            ) {
            ...
            <div id="testSheet" v-if="loadReady">
                <div id="title" class="mt-10 lg:mt-16">
                    <div class="tracking-wide">
                        <router-link
                            to="/"
                            class="text-sky-800 hover:text-sky-900 transition ease-in-out text-sm lg:text-base"
                            >返回首页
                        </router-link>
                    </div>
                    <div
                        class="text-xl lg:text-3xl my-1 lg:my-3 tracking-widest text-amber-800"
                    >
                        您正在完成
                    </div>
                ...
                <div id="problems" class="mt-8 lg:mt-10">
                    <div
                        class="animate__animated backdrop-blur-xl bg-white/75 shadow-xl px-10 py-8 lg:py-10 my-4 lg:my-8 rounded-3xl"
                    ...
                    >
                        <p class="font-mono">
                            ...
                            <span class="text-sm lg:text-lg">
                                / {{ testContent.data.problems.length }}</span
                            >
                        </p>
                        <p class="text-xl lg:text-3xl mt-3 lg:mb-8">
                            {{ testContent.data.problems[currentPage] }}
                        </p>
                        <div
                            id="option"
                        ...
                        >
                            <div
                                v-if="
                                    testContent.data.answers.length !=
                                    testContent.data.problems.length
                                "
                                v-for="optionIndex in testContent.data
                                    .answers[0].length"
                                ...
                                :class="{
                                    optionActive:
                                        userOption[currentPage + 1] ==
                                        optionIndex,
                                }"
                                :data-order="currentPage + 1"
                            ...
                            <div
                                v-else
                                v-for="optionIndex in testContent.data.answers[
                                    currentPage
                                ].length"
                                class="text-sm lg:text-lg text-center rounded-lg border-2 p-1 border-amber-500 transition ease-in-out select-none cursor-pointer hover:bg-orange-100"
                                :class="{
                            ...
                            >
                                {{
                                    testContent.data.answers[currentPage][
                                        optionIndex - 1
                                    ]
                                }}
                            </div>
                ...
                <div
                    class="mt-4 lg:mt-8 flex space-x-1 lg:space-x-3 justify-center"
                >
                    <button
                    ...
                    <button
                        ...
                        @click="nextProblem"
                        :disabled="
                            currentPage + 1 == testContent.data.problems.length
                        "
                    >
                        下一题
                    </button>
